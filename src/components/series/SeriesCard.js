...
import { 
  ...
  Divider, Badge, Stack, CardActionArea 
} from "@mui/material";
...
// シリーズバッジ（右上に表示）
const SeriesBadge = styled(Badge)(({ theme }) => ({
  position: "absolute",
  top: -10,
  right: 16,
  "& .MuiBadge-badge": {
...
const SeriesCard = ({ series }) => {
  ...
  const { _id, title = "無題", description = "説明なし", author = {}, tags = [], posts = [], isAdultContent, isOriginal, aiGenerated } = series;

  return (
    <StyledCard elevation={2}>
      {/* シリーズバッジ（シリーズを示すラベルを右上に表示） */}
